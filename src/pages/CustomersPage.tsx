...
export function CustomersPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    setFormError('')
    if (!name.trim()) { setFormError('Nama tidak boleh kosong.'); return }
    if (!phone.trim()) { setFormError('Nomor HP tidak boleh kosong.'); return }
    ...
    const { error } = await supabase
      .from('customers')
      .insert({ name: name.trim(), phone: phone.trim(), address: address.trim() })

    if (error) {
      setFormError('Gagal menyimpan data pelanggan.')
    } else {
      setName(''); setPhone(''); setAddress(''); setFormError('')
      setShowDialog(false)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="text-sm text-muted-foreground">
        ...
        <span className="font-medium text-foreground">{customers.length}</span> pelanggan
      </div>
        ...
        <CardContent className="p-0">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <p className="text-muted-foreground font-medium">Tidak ada pelanggan</p>
              ...
              <TableBody>
                {filtered.map((c, i) => (
                  <TableRow key={c.id}>
                    ...
                    <TableCell>
                      <a href={`tel:${c.phone}`} className="flex items-center gap-1 text-sm hover:underline text-foreground">
        ...
        <DialogContent className="max-w-sm">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-1.5">
              <Label htmlFor="cust-name">Nama Lengkap <span className="text-destructive">*</span></Label>
              <Input
                id="cust-name"
            ...
            <div className="space-y-1.5">
              <Label htmlFor="cust-phone">Nomor HP <span className="text-destructive">*</span></Label>
              <Input
                id="cust-phone"
