...
export function LoginPage() {
  ...
  const [regRole, setRegRole] = React.useState<'owner' | 'karyawan'>('karyawan')
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (error) setLoginError('Email atau password salah. Silakan coba lagi.')
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    if (error) setRegError(error.includes('already') ? 'Email sudah terdaftar.' : error)
    setRegLoading(false)
  }
    ...
    <div className="min-h-svh flex items-center justify-center bg-muted/30 p-4">
      <div className="w-full max-w-md space-y-6">
        ...
        <div className="flex flex-col items-center gap-3 text-center">
          ...
          <div>
            <h1 className="text-2xl font-bold tracking-tight">Gresik Laundry</h1>
            <p className="text-sm text-muted-foreground">Sistem Informasi Manajemen Laundry</p>
          </div>
        </div>
        ...
        <Card>
          <CardHeader className="pb-4">
            ...
            <CardDescription>Masuk atau daftarkan akun baru untuk melanjutkan</CardDescription>
          </CardHeader>
          <CardContent>
            <Tabs defaultValue="login">
              ...
              <TabsContent value="register">
                <form onSubmit={handleRegister} className="space-y-4">
                  ...
                  <div className="space-y-2">
                    <Label>Peran</Label>
                    <Select value={regRole} onValueChange={(v) => setRegRole(v as 'owner' | 'karyawan')}>
        ...
        <p className="text-center text-xs text-muted-foreground">
          Gresik Laundry &copy; {new Date().getFullYear()} &mdash; Sistem Informasi Manajemen
        </p>
      </div>
    </div>
  )
